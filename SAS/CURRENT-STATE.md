...
- Lead-topology economy modeling — **PARKED (Stephen-ruled 2026-07-20): revisit only if crowd-test/playtest data shows route-dependent stalls** (L6/L7/L8 are parallel — route order changes pinch timing; current model is aggregate only).
...
- **Audience prototype (judging the podcast proposition): NOT READY** — audio opening ✅ and Del ✅ are now live — remaining: VO and FTUE first-merge choreography only (audio t01 RULED FINAL 2026-07-19 — the spec'd pocket case read as cufflinks at low res; bare recognisable earbuds are canon). Testing appeal before these exist risks judging a proposition players never saw. **FTUE choreography SPEC LOCKED 2026-07-20 (Stephen-ruled): two PRE-SEEDED audio T1 items when the goal opens (L1 n3) · SOFT guide (pulse pair + slight dim, input free — no hard lock) · on merge, sparkle then AUTO-PLAY nodes 4–5. Build = CaseFlow wiring in Main Merge; unblocked, awaiting build session.**
