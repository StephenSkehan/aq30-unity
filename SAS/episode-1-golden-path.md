...
Known art/wiring gaps: Dot portraits LIVE (interim keyed set) · Del portrait NOT live (awaits transparent-native regen — navy-on-navy unkeyable) · VO fields empty · FTUE first-merge choreography pending · hamburger menu = drawn placeholder
```

Item shorthand: F=forensic_tools · R=rusty_anchor · D=food_gifts. Tiers are display tiers (T1 = base).
...
| 3 | Three Years of Goodnights | spawned by 1 | 1× F-T4 UV Light + 1× F-T2 Evidence Bag *(P-T2 remapped 2026-07-15)* | 50 CC | fish-hook message beat (worried portrait) |
...
**Dot portraits are LIVE** (interim keyed set imported 2026-07-14; transparent-native regen supersedes eventually). **Del portrait is NOT live** — approved art exists but is unkeyable (navy-on-navy); her nodes show name + line only until the transparent-native regen is delivered. VO fields empty until recording lands. FTUE first-merge choreography (rig merge at ~45s) is a CaseFlow wiring task in the UI phase, not in this data.
