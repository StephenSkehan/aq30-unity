...
- **Kestrel Corner Diner** (granted **L3 e1_archive**; moved from L5 on 2026-08-04 because the new e1_books lead between L4 and L5 requires 2× Paper Cup) → food_gifts (D) — e1_books, L7, L9, L10, L11. *(Variety pass 2026-08-06, Stephen-ruled, tier-exact = T1eq-neutral: L8 Beer Bottle → Broadcast Microphone Rig (audio t4 — recording the marina interviews); L10 UV Light → Burger Single (food t4 — door-knocking Dot's route). Audio now also required at L8.)* **This grant was missing** until 2026-07-12: no lead provisioned the food generator, so L7 "The Man Who Came at Noon" (burger + coffee) was **unsatisfiable — a hard Ep1 progression blocker**. First fixed on `Lead_E1_Pod1` (L5); relocated to `Lead_E1_Archive` (L3) when e1_books landed.
