...
| 9 | Off the Record — **THE BRANCH** | leads 7 AND 8 | 1× F-T4 + 1× D-T2 | 50 CC | **Del Cruz speaks (no portrait yet — name only, expected). CHOICE UI: two buttons. Verify BOTH paths converge; flag set (public/protected)** |
| 10 | Where Dot Went | leads 6 AND 9 | 1× F-T4 + 1× P-T3 Fingerprint Card | 50 CC | **branch variant node**: public run shows "the queue paid its way"; protected shows "Del's quiet trace" — one only |
| 11 | The Hill Cottage | lead 10 | 1× R-T6 Champagne Flute + 1× D-T4 | 185 CC + **2 ingots** | Dot speaks in person (angry beat = her one use); apology; van glimpse |
